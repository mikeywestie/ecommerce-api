...
public class GitHubIssueService {
  ...
  public BugReportResponse createBugReport(BugReportRequest request) {
    ...
    GitHubIssueRequest githubRequest =
        new GitHubIssueRequest(
            buildTitle(request), buildBody(request), List.of("bug", "reported-from-app"));

    GitHubIssueResponse githubResponse =
        restClient
            .post()
  ...
  private String buildTitle(BugReportRequest request) {
    ...
    String message = valueOrDefault(request.message(), "Bug report").replace("\n", " ").trim();
  ...
  private record GitHubIssueRequest(String title, String body, List<String> labels) {}

  private record GitHubIssueResponse(
      Long number, @com.fasterxml.jackson.annotation.JsonProperty("html_url") String htmlUrl) {}
}
