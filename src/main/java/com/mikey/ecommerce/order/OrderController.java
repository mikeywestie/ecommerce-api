...
@Tag(name = "Orders", description = "Order management endpoints for creating orders and retrieving order details.")
public class OrderController {
    ...
    @Operation(summary = "Get all orders", description = "Admin-only endpoint. Returns all customer orders.")
    @ApiResponses(value = {
    ...
    @Operation(summary = "Get my orders", description = "Returns only orders belonging to the authenticated user's email address.")
    ...
    public List<OrderResponse> findMyOrders(Authentication authentication) {
        ...
        return orderRepository.findByCustomerEmailWithItems(authentication.getName())
                .stream()
                .map(OrderMapper::toResponse)
                .toList();
    }

    @Operation(summary = "Get order by ID", description = "Admins can access any order. Customers can only access their own orders.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Order found successfully", content = @Content(mediaType = "application/json", schema = @Schema(implementation = OrderResponse.class))),
