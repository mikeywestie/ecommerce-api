...
public class OrderEventProducer {
  ...
  public void publish(PaymentProcessedEvent event) {
    publishSafely(PAYMENT_PROCESSED_TOPIC, event.orderId().toString(), event);
  }

  public void publish(OrderCreatedEvent event) {
    publishSafely(TOPIC, event.orderId().toString(), event);
  }

  public void publish(CouponAppliedEvent event) {
    publishSafely(COUPON_APPLIED_TOPIC, event.orderId().toString(), event);
  }

  private void publishSafely(String topic, String key, Object event) {
    if (!kafkaEnabled) {
      log.info("Kafka publishing disabled. Skipping event topic={} key={}", topic, key);
      return;
    }
      ...
      kafkaTemplate
          ...
          .whenComplete(
              (result, ex) -> {
                if (ex != null) {
                  log.warn(
                      "Kafka publish failed for topic={} key={}: {}", topic, key, ex.getMessage());
                  return;
                }
